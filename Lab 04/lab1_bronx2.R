...
# Get the number of rows in 'bronxadd'
n_rows <- nrow(bronxadd)

# Use the correct number of rows as the value for 'n' in 'sample.int()'
addsamplerows <- sample.int(n = n_rows, size = nsample, replace = FALSE)

# Use the 'addsamplerows' index to select random rows from 'bronxadd'
addsample <- bronxadd[addsamplerows, ]
...
# Check if both latitude and longitude are not equal to 0
